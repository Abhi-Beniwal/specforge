...
def generate_analysis(
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        ...
        try:
            ...
            logger.warning(
                f"Security Analysis Attempt {attempt} failed: {last_error}"
            )
...
def security_node(state: SpecForgeState) -> SpecForgeState:
    ...
    try:
        ...
        rag_context = get_relevant_context(
            f"security compliance GDPR OWASP requirements for: {state['idea']}"
        )
...
Use the retrieved compliance/security context in your reasoning.
...
"""
        ...
        state["security_scores"] = {
            "threat_severity_score": analysis[
                "threat_severity_score"
            ],
            "production_security_score": analysis[
                "production_security_score"
            ]
        }

        state["critical_vulnerabilities"] = analysis[
            "critical_vulnerabilities"
        ]

        state["security_blockers"] = analysis[
            "security_blockers"
        ]
