        ...
        <div style={{ display:"flex", gap:16, paddingTop:12,
          ...
          {[
            ...
            <div key={label} style={{ display:"flex", flexDirection:"column", gap:2 }}>
              ...
              <span style={{ fontSize:10, color:"#334155", fontFamily:"'IBM Plex Mono',monospace" }}>{val}</span>
            </div>
          ))}
        </div>
      )}
...
// ─── MobileAgentPanel ─────────────────────────────────────────────────────────
// On mobile, agents are shown as a horizontal scrollable tab strip
// instead of a sidebar grid layout.

function MobileAgentPanel({ results, active, setActive, statuses }) {
  return (
    <div style={{ marginBottom:16 }}>
      {/* Horizontal scrollable agent tabs */}
        ...
        {AGENTS.filter(a => results[a.key]).map(agent => {
          ...
          return (
            <button key={agent.key}
              ...
              <span style={{ fontSize:10, fontWeight:600, color:isActive?agent.color:"#475569",
      ...
      {active && (() => {
        ...
        return (
          <div style={{ background:"#07070f", border:"1px solid #0f0f1c", borderRadius:12, overflow:"hidden" }}>
...
export default function Home() {
  ...
  const submit = useCallback(async () => {
    ...
    setProjectId(null);
    setError(null);
    setLoadingText("Waking up backend...");

    try {
      // Wake up Render backend first (prevents mobile Safari "Load failed")
      try {
      ...
      if (err.name === "AbortError") {
        ...
        setError("Connection failed. The backend may be waking up — wait 30 seconds and try again.");
      }
      setPhase("idle");
    }
  }, [idea, phase]);
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
        ...
        /* Hide scrollbar on mobile agent tabs */
      ...
      <div style={{ position:"relative", zIndex:1, minHeight:"100vh", paddingBottom:80 }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:8 }}>
            {isRunning && (
              <div style={{ display:"flex", alignItems:"center", gap:6,
                padding:"4px 10px", background:"#0f0900",
        ...
        <div style={{ maxWidth:1280, margin:"0 auto", padding:`0 ${px}` }}>
          ...
          {/* ── Hero ── */}
          <div style={{
            maxHeight:isIdle ? 700 : 0, opacity:isIdle ? 1 : 0,
            overflow:"hidden",
          ...
          {/* ── Input ── */}
          <div style={{ paddingTop:isIdle ? 0 : 20, paddingBottom:16,
              ...
              <div style={{ borderRadius:13, background:"#08081a", overflow:"hidden" }}>
                <textarea
                  ...
                  style={{ width:"100%", background:"transparent", border:"none",
                    ...
                    padding: isIdle
                      ? (isMobile ? "16px 16px 50px 16px" : "18px 18px 56px 18px")
                      : "14px 16px",
                          ...
                          <div style={{ width:nodeSize, height:nodeSize, borderRadius:"50%",
                            ...
                            {isDn
                              ...
                              ? <span style={{ width:6, height:6, borderRadius:"50%", background:"#f59e0b",
                                  display:"block", animation:"pulseGlow 1s ease infinite" }} />
          ...
          {Object.keys(results).length > 0 && (
            <div className="fade-in" style={{ marginBottom:32 }}>
              {isMobile ? (
                // Mobile: stacked layout with tab strip
                <MobileAgentPanel
                  results={results}
                  active={active}
                  setActive={setActive}
          ...
          {/* ── Completion banner ── */}
          <div style={{
            maxHeight:isDone ? 120 : 0, opacity:isDone ? 1 : 0,
            overflow:"hidden", transition:"max-height .5s ease, opacity .4s ease",
